...
// a) //ergebnis sieht so aus: ("hans", 1, 48)
def maxWorkPerWeek(l:List[(String, Int, List[Int])]):(String,Int,Int) = {
  l.map(x=>(x._1,x._2,x._3.sum)).maxBy(_._3)
...
// b) //ergebnis sieht so aus: ("hans", 148)
def maxWork(l:List[(String, Int, List[Int])]):(String,Int) =
  //l.map(x => (x._1, x._3.sum)).groupBy(_._1).view.mapValues(x => x.maxBy(_._2)).toMap.values.maxBy(_._2)
  l.groupBy(_._1).view.mapValues(_.map(_._3.sum).sum).maxBy(_._2)
maxWork(stundenProtokoll)


// c) // (12, List("Hans","Monika")

def dailyMaxMySolution(l: List[(String, Int, List[Int])]): (Int, List[String]) =
  val maxH = l.flatMap(_._3).max //12
  val names = l.filter(x => x._3.contains(maxH)).map(_._1).distinct // List("Hans","Monika")
  (maxH, names)


def dailyMax(l: List[(String, Int, List[Int])]): (Int, List[String]) = {
  val maxHours = l.flatMap(x => x._3.map(y => (x._1, y))).
    groupBy(_._2).reduce((x, y) => if (x._1 > y._1) x else y)

  (maxHours._1, maxHours._2.map(_._1).distinct)
}


dailyMax(stundenProtokoll)
dailyMaxMySolution(stundenProtokoll)
